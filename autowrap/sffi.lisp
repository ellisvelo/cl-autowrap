...
appropriate."
  ...
  (with-wrap-attempt ("record (~A (~S))" type name) name
    (let* ((record (gethash `(,type (,name)) *foreign-types*)))
      ...
      (unless (or (= 0 bit-size)
		  (not (null (foreign-record-bit-size record))))
        (setf (foreign-record-bit-size record) bit-size))
      (unless (or (= 0 bit-alignment)
		  (not (null (foreign-record-bit-alignment record))))
        (setf (foreign-record-bit-alignment record) bit-alignment))
      (when (and (car field-list)
		 (null (foreign-record-fields record)))
